...
console.log("Complete cards:  "+localStorage.setItem);
...
const cardHierarchy = ["two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "jack", "queen", "king", "ace"];
...
let activePlayers = [0, 1, 2, 3] //Player 0 as you 
...
function evaluateHand(iteration, gameStep) {
    ...
    let ace = 0;
    for (let i = 0; i < cardsArr.length; i++) {
        if (cardHierarchy.indexOf(cardsArr[i].value) !== null) {
            cardIndexes.push(cardHierarchy.indexOf(cardsArr[i].value));
        }
    ...
    for (let i = 0; i < valueArr.length; i++) {
        if (valueArr[i] > 0) {//determine highest card
            highCard = cardHierarchy[i];
            if (gameStep === 1) {
                playerHighCards[iteration] = i;
            }
            if (resultList[iteration] === 0) {
                highCard = " - " + cardHierarchy[valueArr.lastIndexOf(1)];
        ...
        if (valueArr[i] === 2) {//a pair           /*collect pair for later eval*/           
            ...
            cardsInvolved = cardsInvolved + " - " + cardHierarchy[i] + "s";
        }
        if (valueArr[i] === 3) {//three of a kind
            if (resultList[iteration] < 3) {
                ...
                cardsInvolved = cardsInvolved + " - " + cardHierarchy[valueArr.lastIndexOf(3)] + "s";
            }
        }
        if (valueArr[i] === 4) {
            if (resultList[iteration] < 7) {
                ...
                cardsInvolved = cardsInvolved + " - " + cardHierarchy[valueArr.lastIndexOf(4)] + "s";
            }
        }
    }
    ...
    if (valueArr.indexOf(3) !== -1 && valueArr.indexOf(2) !== -1) {    /*checking for full house*/
        if (resultList[iteration] < 6) {
            ...
            cardsInvolved = cardHierarchy[valueArr.lastIndexOf(2)] + "s - " + cardHierarchy[valueArr.lastIndexOf(3)] + "s";
        }
    }
    ...
    if (resultList[iteration] === 0) {
        ...
        HighCardMessage = " - " + cardHierarchy[valueArr.lastIndexOf(1)];
    }
    if (iteration === 0) {
        document.getElementById(playersDetails[iteration]).innerHTML = "You have: " + handHierarchy[resultList[iteration]] + "  " + cardsInvolved + HighCardMessage;
        document.querySelector("#" + playersDetails[iteration]).innerHTML = "You have: " + handHierarchy[resultList[iteration]] + "  " + cardsInvolved + HighCardMessage;/*browser bug fix*/
    }
    if (iteration !== 0 && gameStep === 4 && activePlayers.indexOf[iteration] !== -1) {
        document.getElementById(playersDetails[iteration]).innerHTML = plyr + "Player " + (iteration + 1) + ": " + handHierarchy[resultList[iteration]] + "  " + cardsInvolved + HighCardMessage;
        document.querySelector("#" + playersDetails[iteration]).innerHTML = plyr + "Player " + (iteration + 1) + ": " + handHierarchy[resultList[iteration]] + "  " + cardsInvolved + HighCardMessage;        /*browser bug fix*/
    }
    ...
    if (gameStep === 4 && iteration === lastIteration) {
        ...
        if (getOccurrence(resultList, winningHand) === 1) {
            ...
            if (getOccurrence(compareCards, winningCard) > 1) {
                ...
                for (let i = 0; i < 4; i++) {
                    if (compareCards[i] !== -1) {
                        let tempPlayerCards = [cardHierarchy.indexOf(playersHands[i][0].value), cardHierarchy.indexOf(playersHands[i][1].value)];
...
function deal() {
    ...
    function generatePlayer(iteration) {
        ...
        for (let i = 0; i < playersCards.length; i++) {
            ...
            score = score + cardHierarchy.indexOf(playersCards[i].substring(0, playersCards[i].indexOf("-")));
        }
